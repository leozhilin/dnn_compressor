...
# 添加tensorboard画图可视化
writer = SummaryWriter("./result/mobilenet/mobilenet_origin_model")
...
if __name__ == "__main__":
    ...
    total_train_time = 0
    print("开始训练! mobilenet-v2! 冲!")  # 定义遍历数据集的次数
    ...
    for epoch in range(pre_epoch, EPOCH):
        ...
        if epoch != 0:
            ...
            qd_compressor(quantized_model_old, quantized_model_new, S, Z,
                          path=f"./Snapshots/mobilenet/mobilenet_snapshot_epoch{epoch}",
                          compressed_s_path=f"./scales/mobilenet/scale_epoch{epoch}",
                          compressed_z_path=f"./zero_points/mobilenet/zero_point_epoch{epoch}")
            compress_time_t1 = time.time()
            total_compress_time += (compress_time_t1 - compress_time_t0)
        torch.save(model, f"./model/mobilenet/mobilenet_lr001_epoch{epoch}.pth")
        print("开始测试!")
        with torch.no_grad():
            correct = 0
            total = 0
